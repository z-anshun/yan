...
func (j *Jwt)CreatJwt(name string, id int) string {
	...
	header, err := json.Marshal(&j.Header) //转化为json
	if err != nil {
	...
	j.Signature = base64.StdEncoding.EncodeToString(s)
...
func (j *Jwt)CheckToken(token string) (uid int, name string, err error) {
