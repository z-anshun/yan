		...
		for day, vd := range day {
			...
			if vd[1] != " >" {
				...
				for _, v1 := range all {
					...
					switch k {
					case 0:
						studentClass.Lesson = "一、二节"
						studentClass.Begin_lesson = 1
					case 1:
						studentClass.Lesson = "三、四节"
						studentClass.Begin_lesson = 3
					case 2:
						studentClass.Lesson = "五、六节"
						studentClass.Begin_lesson = 5
					case 3:
						studentClass.Lesson = "七、八节"
						studentClass.Begin_lesson = 7
					case 4:
						studentClass.Lesson = "九、十节"
					...
					if check(thr, v1[0]) {
						...
						switch k {
						case 0:
							studentClass.Lesson = "一、二、三节"

						case 2:
							studentClass.Lesson = "五、六、七节"

						case 4:
							studentClass.Lesson = "九、十、十一节"
					...
					if check(four, v1[0]) {
						...
						switch k {
						case 0:
							studentClass.Lesson = "一、二、三、四节"

						case 2:
							studentClass.Lesson = "五、六、七、八节"

						case 4:
							studentClass.Lesson = "九、十、十一、十二节"
					...
					if len(flag_con) != 0 {
						...
						studentClass.Week = putWeek(s, e, flag)
						//判断后面还有不有
						if len(week) > 2 {
							for k, w := range week {
								if k >= 2 {
									w_i, _ := strconv.Atoi(w[0])
									studentClass.Week = append(studentClass.Week, w_i)
								}
							}
						}
					} else {
